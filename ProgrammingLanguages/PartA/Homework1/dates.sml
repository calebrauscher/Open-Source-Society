...
(* Takes two dates and evaulates as true if first date comes before the second date *)
fun is_older(d1 : int*int*int, d2 : int*int*int) =
    (#1 d1) < (#1 d2) orelse (#2 d1) < (#2 d2) orelse (#3 d1) < (#3 d2)

(* Returns the number of dates that are in the month provided *)
...
(* Returns the number of dates in the list of dates that are in any of the monthd in the list of months *)
...
(* Returns a list holding the dates from the arguement list of dates that are in any of the months in the list of months. *)
...
(* Returns the date in the form of Month Day, Year. *)
...
(* Returns an int n such that the first n elements of the list add to less than sum, but the first n+1 elements of the list add to sum or more. *)
...
(* Returns what month a day of the year is in. *)
fun what_month(day : int) =
    let
	val days_in_month = [31, 28, 31, 39, 31, 30, 31, 31, 30, 31, 30, 31]
    in
...
fun reasonable_date(d : int*int*int) =
    ((#1 d) > 0 andalso (#2 d) >= 1 orelse (#2 d) <= 12) andalso ((#3 d) > 0 andalso (((#2 d) = 2 andalso (#3 d) <= 29) orelse ((#2 d) = 1 orelse (#2 d) = 3 orelse (#2 d) = 5 orelse (#2 d) = 7 orelse (#2 d) = 8 orelse (#2 d) = 10 orelse (#2 d) = 12) andalso (#3 d) <= 31) orelse ((#2 d) = 4 orelse (#2 d) = 6 orelse (#2 d) = 9 orelse (#2 d) = 11) andalso (#3 d) <=30)
